...
public class TicTacToeGame {
    ...
    public boolean validateMoveXY(int row, int column){
        if(validateXAxis(row) && validateYAxis(column)){
            return true;
        }
        return false;
    }
    public boolean validateXAxis(int row){
        //validamos si pertenece al rango X
        if(row >= 0 && row < getTotalRows()){
            return true;
        }
        return  false;
    }
    public boolean validateYAxis(int column){
        //validamos si pertenece al rango Y
        if(column >= 0 && column < getTotalColumns()){
            return true;
        }
        return  false;
    }
